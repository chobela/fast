...
$sqll = "SELECT * FROM debtors LEFT JOIN comments ON debtors.id = comments.debtor_id WHERE client = $clientID AND comments.id IN (SELECT MAX(comments.id) FROM comments GROUP BY comments.debtor_id) GROUP BY comments.debtor_id";
...
function userAction(type,id){
    ...
    $.ajax({
        ...
        success:function(msg){
            if(msg == 'ok'){
                alert('Comment been '+statusArr[type]+' successfully.');
                getUsers();
                $('.form')[0].reset();
                $('.formData').slideUp();
            }else{
